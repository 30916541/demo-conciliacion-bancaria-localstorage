...
export default class Cl_vReporte extends Cl_vGeneral {
    ...
    _lblTotalCargos;
    _lblTotalAbonos;
    _lblPorcentajeCargos;
    _lblPorcentajeAbonos;
    _divCategoriasGrid;
    constructor(controlador) {
        ...
        this._lblSaldoInicial = this.crearHTMLElement("saldoInicial");
        this._lblSaldoFinal = this.crearHTMLElement("saldoFinal");
        this._lblTotalCargos = this.crearHTMLElement("totalCargos");
        this._lblTotalAbonos = this.crearHTMLElement("totalAbonos");
        this._lblPorcentajeCargos = this.crearHTMLElement("porcentajeCargos");
        this._lblPorcentajeAbonos = this.crearHTMLElement("porcentajeAbonos");
        this._divCategoriasGrid = document.getElementById("categoriasGrid");
    }
    mostrar(saldoInicial, movimientos) {
        this.ocultarTodo();
        if (this.vista) {
    ...
    cargarReporte(saldoInicial, movimientos) {
        const totalAbonos = movimientos
            .filter((m) => m.tipo === "Abono")
            .reduce((sum, m) => sum + Number(m.abono || 0), 0);
        const totalCargos = movimientos
            .filter((m) => m.tipo === "Cargo")
            .reduce((sum, m) => sum + Number(m.cargo || 0), 0);
    ...
    cargarMontosPorCategoria(movimientos) {
        if (!this._divCategoriasGrid)
            return;
